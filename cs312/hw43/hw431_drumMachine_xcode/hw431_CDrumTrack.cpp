...
void CDrumTrack::write_onebeat(std::string pat)
{
    for (int i = 0; i < pat.size(); ++i) {
        ...
        // decide whether to push a non
        if (current_pat == '1') {

            int current_dur = vbeat_dur.at(i);
            uint32_t current_tm = beat_num * 1000 + i * 250;
            push_non(current_tm, 9, drum_note, 127);
            push_nof(current_tm + current_dur - 1, 9, drum_note);
