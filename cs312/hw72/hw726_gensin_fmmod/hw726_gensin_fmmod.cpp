...
// I:     modulationIndex
  ...
  MY_TYPE carF = k2PI * Fc * T;
  MY_TYPE carM = k2PI * Fm * T;
  MY_TYPE I = dFm / Fm;

  for (int n = 0; n < secs * FS; n++) {
    MY_TYPE samp = A * sin(carF * n + I * sin(carM * n));
    vout.push_back(samp);  
  }
...
int main(int argc, char *argv[])
{
 ...
 vout.clear();
 vout = fm_modulation(0.5, 800, 10, 1000, 2.0);
 write_fm_file("my_example_1.wav", vout);

 vout.clear();
 vout = fm_modulation(0.9, 1000, 100, 500, 2.0);
 write_fm_file("my_example_2.wav", vout);

 vout.clear();
 vout = fm_modulation(0.1, 300, 10, 100, 2.0);
 write_fm_file("my_example_3.wav", vout);
